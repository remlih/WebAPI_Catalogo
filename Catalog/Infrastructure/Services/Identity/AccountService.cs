...
namespace Catalog.Infrastructure.Services.Identity
{
    public class AccountService : IAccountService
    {
        ...
        public async Task<IResult> ChangePasswordAsync(ChangePasswordRequest model, string userId)
        {
            ...
            if (user == null)
            {
                return await Result.FailAsync(_localizer["Usuario no encontrado."]);
            }
        ...
        public async Task<IResult> UpdateProfileAsync(UpdateProfileRequest request, string userId)
        {
            if (!string.IsNullOrWhiteSpace(request.PhoneNumber))
            {
                ...
                {
                    return await Result.FailAsync(string.Format(_localizer[$"El número de teléfono {request.PhoneNumber} ya está en uso."]));
                }
            }
            ...
            {
                return await Result.FailAsync(string.Format(_localizer[$"El correo electrónico {request.Email} ya está en uso."]));
            }
        }
        ...
        public async Task<IResult<string>> GetProfilePictureAsync(string userId)
        {
            ...
            if (user == null)
            {
                return await Result<string>.FailAsync(_localizer["Usuario no encontrado"]);
            }
        ...
        {
            var user = await _userManager.FindByIdAsync(userId);
            if (user == null) return await Result<string>.FailAsync(message: _localizer["Usuario no encontrado"]);
