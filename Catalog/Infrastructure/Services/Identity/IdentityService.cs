...
namespace Catalog.Infrastructure.Services.Identity
{
    ...
    {
        private const string InvalidErrorMessage = "Correo electrónico o contraseña no válidos.";
        ...
        public IdentityService(
            UserManager<CatalogUser> userManager, RoleManager<CatalogRole> roleManager,
            IOptions<AppConfiguration> appConfig, SignInManager<CatalogUser> signInManager,
        ...
        public async Task<Result<TokenResponse>> LoginAsync(TokenRequest model)
        {
            ...
            if (user == null)
            {
                return await Result<TokenResponse>.FailAsync(_localizer["Usuario no encontrado."]);
            }
            if (!user.IsActive)
            {
                return await Result<TokenResponse>.FailAsync(_localizer["El usuario no está activo. Póngase en contacto con el administrador."]);
            }
            if (!user.EmailConfirmed)
            {
                return await Result<TokenResponse>.FailAsync(_localizer["Correo electrónico no confirmado."]);
            }
            var passwordValid = await _userManager.CheckPasswordAsync(user, model.Password);
            if (!passwordValid)
            {
                return await Result<TokenResponse>.FailAsync(_localizer["Credenciales no válidas."]);
            }
        ...
        public async Task<Result<TokenResponse>> GetRefreshTokenAsync(RefreshTokenRequest model)
        {
            if (model is null)
            {
                return await Result<TokenResponse>.FailAsync(_localizer["Token de cliente no válido."]);
            }
        ...
        private async Task<string> GenerateJwtAsync(CatalogUser user)
        {
            var token = GenerateEncryptedToken(GetSigningCredentials(), await GetClaimsAsync(user));
            return token;
        }

        private async Task<IEnumerable<Claim>> GetClaimsAsync(CatalogUser user)
        {
        ...
        private ClaimsPrincipal GetPrincipalFromExpiredToken(string token)
        {
            ...
            {
                throw new SecurityTokenException(_localizer["Token no valido"]);
            }
